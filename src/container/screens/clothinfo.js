...
export default class Clothinfo extends React.Component {
    constructor(props) {
        super(props)
        console.log("clothsinfo",props.route.params)
        this.state = {
            userData: {},
        }
    }
    componentDidMount(){
        fetch(`http://handout.pythonanywhere.com/api/donate/GetClothInfo/${this.props.route.params.id}`)
        .then(res=>res.json())
        .then(resjson=>{
                ...
                <View style={styles.bottombox}>
                    <View style={{justifyContent:"space-between"}}>
                        <View style ={styles.profile}>
                            <Image source={cloths} style={{height: hDP('9%'), width: wDP('16%'),}}/>
                        </View>
                    ...
                    <View style={styles.info}>
                        ...
                        <View style={{flexDirection:"row"}}>
                            <Text style={styles.text}>Cloths Type:</Text>
                            <Text style={styles.text1}>{this.state.userData.clothsType}</Text>
                        </View>
                        <View style={{flexDirection:"row"}}>
                            <Text style={styles.text}>Cloths Name:</Text>
                            <Text style={styles.text1}>{this.state.userData.clothsName}</Text>
                        </View>
                        <View style={{flexDirection:"row"}}>
                        ...
                        <View style={{flexDirection:"row"}}>
                            ...
                            <Text style={styles.text1}>{this.state.userData.clothsDescription}</Text>
                        </View>                     
                    </View>
                </View>
            </View>
...
const styles = StyleSheet.create({
    topbox: {
        width: wDP('100%'),
        height: hDP('20%'),
        backgroundColor: AppColor.colors.LitePink,
        opacity: 0.8,
        alignItems:'flex-end',
    },
    view: {
