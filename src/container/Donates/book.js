...
donate = async() => {
  console.log(this.state.donarname, this.state.donarmobile,this.state.donaraddress,
              this.state.landmark, this.state.booksType,this.state.booksName,
    ...
    if (this.state.donarmobile != '') {
        if (this.state.donaraddress != '') {
            if (this.state.landmark != '') {
                if (this.state.booksType != '') {
                    if (this.state.booksName != '') {
                                        ...
                                        let body = {
                                          ...
                                          headers:{
                                           ...
                                           body:JSON.stringify({
                                            ...
                                            booksDescription:this.state.description,
                                            latitude:this.state.latitude,
                                            longititude:this.state.longitude
                                           })
                                         }
                                        let result = await fetch('http://handout.pythonanywhere.com/api/donate/DonateBook/',body)
    ...
    return (
     <Container>
        <Header>
          ...
          <Body>
            <Title style={{ width: 300, alignItems: 'center' }}>Donate Books Details</Title>
          </Body>
          <Right />
        </Header>
        <Content>
          <ScrollView>
            <Content style={{ padding: 20 }}>
              ...
              <Label style={{ marginLeft: 10, marginTop: 10, fontWeight: 'bold' }}  >Books Type</Label>
              <Item>
                <Picker
                  mode="dropdown"
                  ...
                  selectedValue={this.state.booksType}
                  onValueChange={this.onValueChange.bind(this)}
                >
                  <Picker.Item label="Select Dress Type" value="key0" />
                  <Picker.Item label="Engineering" value="Engineering" />
                  <Picker.Item label="Arts" value="Arts" />
                  <Picker.Item label="SchoolBooks" value="SchoolBooks" />
                  <Picker.Item label="Novels" value="Novels" />
                  <Picker.Item label="Others" value="Others" />
                </Picker>
              </Item>
              <Label style={{ marginLeft: 10, marginTop: 10, fontWeight: 'bold' }}  >Books Name</Label>
              <Item>
                <Input placeholder='Ex:Lost in Math etc' onChangeText={booksName => this.setState({ booksName })} maxLength={200} />
